...
export class ResultsComponent implements OnInit {
  ...
  getWeather(lat: string, lon: string, cnt: string){
    ...
    this.weatherService.getWeatherForecast(lat, lon, cnt).subscribe((data: any)=> {
      ...
      this.weatherInfo.temperature = (data.list[0].main.temp-this.kelvinScale).toPrecision(2);
      this.weatherInfo.felt_temperature = (data.list[0].main.feels_like-this.kelvinScale).toPrecision(2);
